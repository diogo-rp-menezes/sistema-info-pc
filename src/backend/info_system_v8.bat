...
:: 4. MEMÓRIA RAM (versão completa em PowerShell)
echo     "memory": [ >> "%output_file%"
powershell -command "$memory = @(); Get-WmiObject Win32_PhysicalMemory | ForEach-Object { $module = @{ size_gb = [math]::Round($_.Capacity/1GB); Manufacturer = $_.Manufacturer; PartNumber = $_.PartNumber; SerialNumber = $_.SerialNumber }; $memory += $module }; $memory | ConvertTo-Json -Compress | Out-File -FilePath \"%output_file%\" -Append -Encoding ASCII" >nul
echo     ], >> "%output_file%"

:: 5. DISCOS (corrigido - removido colchetes extras)
...
:: 8. INFORMAÇÕES MANUAIS (nova seção)
